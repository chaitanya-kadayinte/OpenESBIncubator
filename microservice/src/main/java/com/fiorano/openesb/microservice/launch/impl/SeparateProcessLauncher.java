...
public class SeparateProcessLauncher implements Launcher<SeparateProcessRuntimeHandle> {
    ...
    public MicroserviceRuntimeHandle launch(final LaunchConfiguration launchConfiguration, final String configuration) throws Exception {
        final String name = launchConfiguration.getServiceName();
        final String applicationName = launchConfiguration.getApplicationName();
        final String applicationVersion = launchConfiguration.getApplicationVersion();
                ...
                if(controlEvent instanceof DataRequestEvent && controlEvent.isReplyNeeded()) {
                    for(DataRequestEvent.DataIdentifier request: ((DataRequestEvent) controlEvent).getDataIdentifiers()) {
                        if(request == DataRequestEvent.DataIdentifier.COMPONENT_CONFIGURATION) {
                            ...
                            ccpEventManager.getCcpEventGenerator().sendEvent(dataEvent, LookUpUtil.getServiceInstanceLookupName(applicationName,applicationVersion,name));
                            break;
                        }
                    }
                }
